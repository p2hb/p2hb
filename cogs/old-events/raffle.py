...
class Raffle(commands.Cog):
    ...
    async def raffle(self, ctx):
        ...
        ticket_count = await self.bot.mongo.db.raffle.count_documents(
            {"raffle_type": "lando"}
        )
        embed.add_field(name="Price per ticket:", value=f"2000 tokens", inline=False)
        embed.add_field(
            name="Total Number of Tickets Bought:",
        ...
        embed.add_field(
            ...
            value="\n".join(
                (
                    f"`{ctx.prefix}raffle tickets` to check your tickets",
                    f"`{ctx.prefix}raffle buy <amount>` to buy tickets",
                )
            ),
        )
    ...
    async def buy(self, ctx, amount=1):
        ...
        if ctx.channel.id not in (828448803245129738, 819320462538440765):
            return await ctx.send(
                f"Please use <#828448803245129738> instead! Run `{ctx.prefix}invite` to join the official server."
            )
        ...
        if amount == 11:
            await ctx.send(
                f"<@!{ctx.author.id}> You spent **{cost:,} tokens** to buy **10 + 1 raffle ticket(s)**! \n \n **Tip:** Run `{ctx.prefix}raffle tickets` to check your tickets"
            )
        else:
            await ctx.send(
                f"<@!{ctx.author.id}> You spent **{cost:,} tokens** to buy **{amount} raffle ticket(s)**! \n \n **Tip:** Run `{ctx.prefix}raffle tickets` to check your tickets"
            )
